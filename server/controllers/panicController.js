...
const { sendPanicEmailToAdmins, sendPanicSMSToAdmins } = require('../services/notificationService');
...
const createPanicRequest = async (req, res) => {
  try {
    ...
    if (adminEmails.length > 0) {
      sendPanicEmailToAdmins(adminEmails, panicData).catch(console.error);
    }
    if (adminPhones.length > 0) {
      sendPanicSMSToAdmins(adminPhones, panicData).catch(console.error);
    }

    res.status(201).json({
      success: true,
      message: 'Panic request sent. Help is on the way.',
      panic,
    });
  } catch (error) {
...
const getPanicRequests = async (req, res) => {
  try {
    ...
    res.json({ success: true, requests, pagination: { total, page: parseInt(page) } });
  } catch (error) {
...
module.exports = { createPanicRequest, getPanicRequests, updatePanicStatus, getMyPanicRequests };